...
{
    return [self initWithCornerRadiusAdvance:cornerRadius
                              rectCornerType:rectCornerType];
}
/** 创建一个圆形imgV */
-(instancetype)pp_imgVMakeRounded
{
    return [self initWithRoundingRectImageView];
}
...
-(void)pp_imgVCornerRadius:(CGFloat)cornerRadius
            rectCornerType:(UIRectCorner)rectCornerType
{
    [self zy_cornerRadiusAdvance:cornerRadius
                  rectCornerType:rectCornerType];
}
...
{
    [self zy_attachBorderWidth:BorderWidth
                         color:borderColor];
}
