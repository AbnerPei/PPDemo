...
NSString * _Nonnull const PPStringIsValidTypePhoneNO   = @"PPStringIsValidTypePhoneNO";
NSString * _Nonnull const PPStringIsValidTypeEmail     = @"PPStringIsValidTypeEmail";
NSString * _Nonnull const PPStringIsValidTypeIDCard    = @"PPStringIsValidTypeIDCard";
NSString * _Nonnull const PPStringIsValidTypeBankCard  = @"PPStringIsValidTypeBankCard";

@implementation NSString (IsValid)
#pragma mark --- 验证字符串是否是【手机号码】
+(BOOL)pp_string_isPhoneNO:(NSString *)str
{
    return [NSString pp_string_isValidWithValidStyle:PPStringIsValidStylePhoneNO str:str];
}
#pragma mark --- 验证字符串是否是【邮箱】
+(BOOL)pp_string_isEmail:(NSString *)str
{
    return [NSString pp_string_isValidWithValidStyle:PPStringIsValidStyleEmail str:str];
}
//必须满足以下规则
...
{
    if ([validIdentifier isEqualToString:PPStringIsValidTypePhoneNO] || [validIdentifier isEqualToString:PPStringIsValidTypeEmail]) {
        NSPredicate *predicate = [NSPredicate pp_predicate_easyMakeWithValidIdentifier:validIdentifier];
        return [predicate evaluateWithObject:str];
    }else if ([validIdentifier isEqualToString:PPStringIsValidTypeIDCard]){
        return [NSString pp_string_isIDCard:str];
    }else if ([validIdentifier isEqualToString:PPStringIsValidTypeBankCard]){
        return [NSString pp_string_isBankCard:str];
    }
    return NO;
}
+(BOOL)pp_string_isValidWithValidStyle:(PPStringIsValidStyle)validStyle str:(NSString *)str
{
    if (validStyle == PPStringIsValidStylePhoneNO || validStyle == PPStringIsValidStyleEmail) {
        NSPredicate *predicate = [NSPredicate pp_predicate_easyMakeWithValidStyle:validStyle];
        return [predicate evaluateWithObject:str];
